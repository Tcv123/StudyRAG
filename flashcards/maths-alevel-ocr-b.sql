...
--
-- RUN ONCE. These are plain INSERTs with no de-duplication, so
-- running the file twice would create a second copy of every card.
-- To check first, count the existing rows for this subject, board and
-- level -- it should be 0 before running and 400 after.
--
-- No card text contains a semicolon, so clients that split statements
-- on a semicolon cannot cut a statement in half.
...
INSERT INTO flashcards (subject, exam_board, topic_id, topic_name, front, back, card_order, level) VALUES
  ...
  ('Mathematics', 'OCR B', '1.3', 'Functions', 'How are the graphs of f and f⁻¹ related?', 'Each is the reflection of the other in the line y = x. Domain and range swap, and for an INCREASING function any intersection lies on y = x.', 12, 'alevel'),
...
INSERT INTO flashcards (subject, exam_board, topic_id, topic_name, front, back, card_order, level) VALUES
  ...
  ('Mathematics', 'OCR B', '1.5', 'Coordinate Geometry', 'Write down the parametric equations of a circle.', 'x = a + r cos θ and y = b + r sin θ, for centre (a, b) and radius r. Eliminating θ with cos²θ + sin²θ = 1 returns the Cartesian form.', 16, 'alevel');
...
INSERT INTO flashcards (subject, exam_board, topic_id, topic_name, front, back, card_order, level) VALUES
  ...
  ('Mathematics', 'OCR B', '1.8', 'Exponentials & Logarithms', 'What do the graphs of y = aˣ and y = log_a x look like?', 'y = aˣ passes through (0, 1) with the x-axis as an asymptote. y = log_a x is its reflection in y = x, through (1, 0) with the y-axis as an asymptote. Change of base is EXCLUDED from H640.', 5, 'alevel'),
...
INSERT INTO flashcards (subject, exam_board, topic_id, topic_name, front, back, card_order, level) VALUES
  ...
  ('Mathematics', 'OCR B', '1.9', 'Differentiation', 'How do you differentiate an inverse function?', 'Use dy/dx = 1 ÷ (dx/dy). For y = ln x, write x = e^y, so dx/dy = e^y = x and dy/dx = 1/x.', 11, 'alevel'),
...
INSERT INTO flashcards (subject, exam_board, topic_id, topic_name, front, back, card_order, level) VALUES
  ...
  ('Mathematics', 'OCR B', '2.2', 'Data Presentation & Interpretation', 'What is a cumulative frequency diagram used for?', 'Plot cumulative frequency against the UPPER class boundary, then read off estimates: the median at n/2, the quartiles at n/4 and 3n/4. Comparative pie charts are EXCLUDED from H640.', 4, 'alevel'),
...
INSERT INTO flashcards (subject, exam_board, topic_id, topic_name, front, back, card_order, level) VALUES
  ...
  ('Mathematics', 'OCR B', '2.3', 'Probability', 'Is P(A | B) the same as P(B | A)?', 'No — they are different quantities and confusing them is a classic error. Reversing one into the other by formula (given extra information) is EXCLUDED from H640. You calculate conditional probabilities from a tree, a two-way table or a Venn diagram.', 14, 'alevel'),
  ('Mathematics', 'OCR B', '2.3', 'Probability', 'Why can a 95% accurate test give mostly false positives?', 'The small false-positive rate applies to the large healthy majority. In 10 000 people with 1% prevalence: 95 true positives against 495 false ones, so only about 16% of positives are genuine.', 15, 'alevel'),
...
INSERT INTO flashcards (subject, exam_board, topic_id, topic_name, front, back, card_order, level) VALUES
  ...
  ('Mathematics', 'OCR B', '2.4', 'Probability Distributions', 'What does X ~ N(μ, σ²) mean, and what is the commonest error?', 'X is Normally distributed with mean μ and VARIANCE σ². Reading the second parameter as the standard deviation is the usual slip — N(100, 25) has σ = 5.', 10, 'alevel'),
...
INSERT INTO flashcards (subject, exam_board, topic_id, topic_name, front, back, card_order, level) VALUES
  ...
  ('Mathematics', 'OCR B', '2.5', 'Statistical Hypothesis Testing', 'Is the Central Limit Theorem required for H640?', 'No — it is listed as an exclusion. You need only know that for X ~ N(μ, σ²) the sample mean is Normal with mean μ and variance σ²/n. A large sample is quoted as a CONDITION for the test, not justified by the theorem.', 13, 'alevel'),
  ...
  ('Mathematics', 'OCR B', '2.5', 'Statistical Hypothesis Testing', 'Write down the test statistic for a Normal test on a mean.', 'z = (x̄ − μ)/(σ/√n). Dividing by σ instead of σ/√n is the usual slip, and it shrinks z, making real effects harder to detect.', 15, 'alevel'),
...
INSERT INTO flashcards (subject, exam_board, topic_id, topic_name, front, back, card_order, level) VALUES
  ...
  ('Mathematics', 'OCR B', '3.3', 'Kinematics in 2 Dimensions', 'How do you find the relative position of one particle from another?', 'The position of B relative to A is r_B − r_A. Note that relative VELOCITY in two dimensions is excluded from H640 — it appears only in the one-dimensional work.', 9, 'alevel'),
...
INSERT INTO flashcards (subject, exam_board, topic_id, topic_name, front, back, card_order, level) VALUES
  ...
  ('Mathematics', 'OCR B', '3.5', 'Forces', 'Why can a normal contact force never be negative?', 'A surface can only push, never pull. A negative value means the object has already left the surface, so the model no longer applies. The term ''angle of friction'' is excluded from H640.', 14, 'alevel'),
...
INSERT INTO flashcards (subject, exam_board, topic_id, topic_name, front, back, card_order, level) VALUES
  ...
  ('Mathematics', 'OCR B', '3.7', 'Rigid Bodies', 'How do you fix the sense of each moment in an equation?', 'Choose one sense as positive, usually anticlockwise, and give every moment that sign or its opposite. In equilibrium the total clockwise moment about a point equals the total anticlockwise moment.', 16, 'alevel');
