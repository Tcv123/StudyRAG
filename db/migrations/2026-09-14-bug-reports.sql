-- ═══════════════════════════════════════════════════════════════════════
-- bug_reports — the list behind admin-bugs.html — 2026-09-14
--
-- WHY THIS EXISTS. admin-bugs.html shipped with its findings hard-coded in
-- a FINDINGS array. That was fine as a record of one audit and useless as a
-- place to put the next bug: adding one meant editing and redeploying the
-- page, and there was nowhere to say which of them matters most.
--
-- This moves the list into a table so it can be added to and ranked from
-- the page itself.
--
-- RANK. A plain integer, ascending, lowest first. Seeded in tens so there
-- is room to drop something between two rows without renumbering. The page
-- reorders by swapping the rank of two neighbours, which is one round trip
-- and cannot leave a gap behind.
--
-- STATUS. 'fixed' rows are kept, never deleted — the page hides them behind
-- a toggle. What was wrong is worth more than a short list.
--
-- ACCESS. Admin-only, enforced here rather than in the page. is_admin() is
-- the same SECURITY DEFINER helper admin.html and admin-attribution.html
-- use (db/migrations/2026-09-11-admin-security.sql), so a non-admin who
-- opens the page gets an empty list no matter what the client does.
--
-- NOT SECRET, THOUGH. Rows seeded below describe bugs in this app. Anyone
-- who can read the deployed JS can see the page; the gate keeps the list
-- out of casual view, it does not make it confidential.
--
-- How to apply: paste into the Supabase SQL editor and Run. Idempotent —
-- the seed is keyed on ref and will not duplicate on a second run.
-- ═══════════════════════════════════════════════════════════════════════

create table if not exists public.bug_reports (
  id         uuid        primary key default gen_random_uuid(),
  -- Short human handle: R-01, A-07, or whatever you type for a new one.
  -- Unique so the seed below can be re-run without duplicating.
  ref        text        not null unique check (length(trim(ref)) between 1 and 24),
  ...
  -- 'live' = reproduced in the browser or measured against the database.
  -- 'code' = read from source.
  verified   text        not null default 'code' check (verified in ('live','code')),
  area       text,       -- the files it lives in
  detail     text,       -- what actually goes wrong
  evidence   text,       -- optional measurement, shown verbatim in a mono block
  fix        text,       -- what to do about it
  rank       integer     not null default 1000,
  created_at timestamptz not null default now(),
  updated_at timestamptz not null default now()
);

create index if not exists idx_bug_reports_rank on public.bug_reports (status, rank);

-- Keep updated_at honest without the client having to remember.
create or replace function public.touch_bug_reports()
...
drop trigger if exists trg_touch_bug_reports on public.bug_reports;
create trigger trg_touch_bug_reports
  before update on public.bug_reports
  for each row execute function public.touch_bug_reports();

alter table public.bug_reports enable row level security;

drop policy if exists "admins read bugs"  on public.bug_reports;
create policy "admins read bugs"  on public.bug_reports
  for select using (public.is_admin());

drop policy if exists "admins write bugs" on public.bug_reports;
create policy "admins write bugs" on public.bug_reports
...
written lands on red   -> max  9   green needs 8/9  = 89%', 'Make THRESHOLD a fraction of the achievable maximum, or stop randomising which tier the written question replaces.', 60),
  ('A-04', '2,471 mark schemes print a literal backslash-n instead of breaking lines', 'amber', 'open', 'live', 'questions/maths-aqa/* · questions/physics/* · questions/maths-edexcel/* · questions/further-maths-edexcel/*', 'Mark schemes render as one run-on paragraph with visible escape characters. The CSS is fine — .mark-scheme-text already has white-space: pre-line. The data is double-escaped, holding backslash-n as two characters rather than a newline. That is effectively all of Maths AQA practice and all of Physics Edexcel.', '1265  questions/maths-aqa/maths-aqa-practice.js
...
class_members: joined_at 2026-09-14T12:40:24Z   <- it worked', 'Fixed 14 Sep 2026 — migration applied. Verified: success screen renders, and a bad code is refused on the 11th attempt in 15 minutes, so the rate limit is live.', 310),
  ('R-04', 'Switching to a teacher account deleted the student’s subjects and grades', 'red', 'fixed', 'live', 'teacher-subjects-sync.js · settings.html:458', 'The sync reconciled user_subjects to exactly the classes owned, deleting everything else — under a label reading "Changes which pages you see. Nothing you’ve already saved is deleted." A student with four subjects who switched to teacher and made one class was left with one row, and even that had lost both grades. Switching back restored nothing.', 'before   Mathematics/AQA C-A, Computer Science/AQA C-A,
