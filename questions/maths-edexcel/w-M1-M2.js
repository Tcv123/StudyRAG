const EDEXCEL_WRITTEN_M1_M2 = {
  ...
  'M2': {
    ...
    red: [
      {
        q: 'Particle P is projected vertically upward from ground level with speed \\(25 \\text{ m s}^{-1}\\) at time \\(t = 0\\). Particle Q is projected vertically upward from the same point with speed \\(15 \\text{ m s}^{-1}\\) at time \\(t = T\\). They collide at height \\(10\\) m, with Q still moving upwards. Find \\(T\\) and the time of collision. Use \\(g = 9.8 \\text{ m s}^{-2}\\).',
        marks: 8,
        tier: 'red',
        diagram: false,
        modelAnswer: 'First check Q can reach 10 m: the maximum height of Q is \\(\\dfrac{15^2}{2(9.8)} = 11.48\\) m \\(>10\\) m, so a collision at 10 m is possible.\n\nParticle P at height 10 m: \\(10 = 25t_P - 4.9t_P^2\\), so \\(4.9t_P^2 - 25t_P + 10 = 0\\) and \\(t_P = \\dfrac{25 \\pm \\sqrt{625-196}}{9.8} = \\dfrac{25 \\pm \\sqrt{429}}{9.8}\\). This gives \\(t_P = 0.438\\) s (going up) or \\(t_P = 4.664\\) s (coming down).\n\nParticle Q at height 10 m, \\(\\tau\\) seconds after its own launch: \\(4.9\\tau^2 - 15\\tau + 10 = 0\\), so \\(\\tau = \\dfrac{15 \\pm \\sqrt{225-196}}{9.8} = \\dfrac{15 \\pm \\sqrt{29}}{9.8}\\). Q is still going up, so \\(\\tau = 0.981\\) s.\n\nSince \\(T = t_P - \\tau\\) must be positive, \\(t_P\\) must be the larger root: \\(t_P = 4.664\\) s (P is on the way down).\n\n\\(T = 4.664 - 0.981 = 3.68\\) s (3 s.f.), and the collision occurs at \\(t = 4.66\\) s after P was launched.'
      },
      {
      ...
      {
        q: 'Particle A moves along the x-axis with displacement \\(x_A = 3t^2 - t^3\\) (m) for \\(t \\geq 0\\). Particle B starts from rest at \\(x = 0\\) at \\(t = 2\\) s and moves with constant acceleration \\(a\\) m s\\(^{-2}\\) (\\(a\\) may be negative). Find the value of \\(a\\) such that B catches A at \\(t = 5\\) s, and verify that B is behind A for \\(2 < t < 5\\).',
        marks: 8,
        tier: 'red',
        diagram: false,
        modelAnswer: 'The velocity of A is \\(v_A = 6t - 3t^2 = 3t(2-t)\\), so A moves forward until \\(t = 2\\) s, reaching \\(x_A = 12-8 = 4\\) m, and then moves back along the axis.\n\nAt \\(t = 5\\): \\(x_A = 3(25) - 125 = -50\\) m.\n\nB has been moving for \\(\\tau = 5-2 = 3\\) s, starting from rest at \\(x=0\\): \\(x_B = \\tfrac{1}{2}a(3)^2 = 4.5a\\).\n\nFor B to catch A: \\(4.5a = -50\\), so \\(a = -\\dfrac{100}{9} \\approx -11.1\\) m s\\(^{-2}\\) (B accelerates in the negative direction).\n\nVerification for \\(2 < t < 5\\): \\(x_B = -\\dfrac{50}{9}(t-2)^2\\). At \\(t = 3\\): \\(x_A = 0\\), \\(x_B = -5.56\\). At \\(t = 3.5\\): \\(x_A = -6.13\\), \\(x_B = -12.5\\). At \\(t = 4\\): \\(x_A = -16\\), \\(x_B = -22.2\\). In every case \\(x_A > x_B\\), so B is behind A (further in the negative direction) throughout \\(2 < t < 5\\), meeting A only at \\(t = 5\\).'
      },
      {
      ...
      {
        q: 'A particle is projected vertically upward with speed \\(U\\). A second particle is projected vertically upward from the same point with speed \\(V > U\\) at time \\(t = T\\) later. Show that, if they collide, the collision occurs at time \\(\\tau = \\dfrac{VT + \\frac{1}{2}gT^2}{V - U + gT}\\) after the first launch, and find the condition on \\(T\\) for the collision to occur above the launch point.',
        marks: 8,
        tier: 'red',
        diagram: false,
        modelAnswer: 'Let \\(\\tau\\) be the time elapsed since particle 1 was launched.\nParticle 1: \\(y_1 = U\\tau - \\tfrac{1}{2}g\\tau^2\\).\nParticle 2 (in flight for \\(\\tau - T\\)): \\(y_2 = V(\\tau-T) - \\tfrac{1}{2}g(\\tau-T)^2\\).\n\nSetting \\(y_1 = y_2\\) and expanding the right-hand side:\n\\(U\\tau - \\tfrac{1}{2}g\\tau^2 = V\\tau - VT - \\tfrac{1}{2}g\\tau^2 + gT\\tau - \\tfrac{1}{2}gT^2\\).\n\nThe \\(\\tfrac{1}{2}g\\tau^2\\) terms cancel: \\(U\\tau = V\\tau - VT + gT\\tau - \\tfrac{1}{2}gT^2\\), so \\(\\tau(V - U + gT) = VT + \\tfrac{1}{2}gT^2\\) and\n$$\\tau = \\frac{VT + \\tfrac{1}{2}gT^2}{V - U + gT}$$\nas required (the denominator is positive since \\(V>U\\) and \\(T>0\\)).\n\nFor the collision to be above the launch point we need \\(y_1 > 0\\), i.e. \\(U\\tau > \\tfrac{1}{2}g\\tau^2\\), i.e. \\(\\tau < \\dfrac{2U}{g}\\) — particle 1 must not yet have landed.\n\nSubstituting and clearing the (positive) denominator: \\(g\\left(VT + \\tfrac{1}{2}gT^2\\right) < 2U(V - U + gT)\\), which rearranges to \\(\\tfrac{1}{2}g^2T^2 + g(V-2U)T - 2U(V-U) < 0\\). Multiplying by \\(2/g^2\\) gives \\(T^2 + \\dfrac{2(V-2U)}{g}T - \\dfrac{4U(V-U)}{g^2} < 0\\). The discriminant simplifies neatly, since \\((V-2U)^2 + 4U(V-U) = V^2\\), so the roots are \\(T = \\dfrac{2U}{g}\\) and \\(T = \\dfrac{2(U-V)}{g} < 0\\).\n\nHence the condition is \\(0 < T < \\dfrac{2U}{g}\\): the second particle must be launched before the first one lands.'
      },
      {
