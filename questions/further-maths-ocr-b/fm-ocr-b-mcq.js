...
'1.1': {
  ...
  green: [
    ...
    { q: 'A proof by contradiction begins by assuming:', options: ['The statement holds', 'A counterexample to the statement already exists', 'The statement is true', 'The negation of what is to be proved'], answer: 3 },
    ...
    { q: 'Direct proof of "\\(n\\) even \\(\\Rightarrow n^2\\) even" writes \\(n=2k\\) and concludes:', options: ['\\(n^2=2k\\)', '\\(n^2=k^2\\)', '\\(n^2=4k\\), hence divisible by 4', '\\(n^2=2(2k^2)\\), hence even'], answer: 3 },
  ],
  amber: [
    { q: 'To prove \\(\\sum_{r=1}^{n}r=n(n+1)/2\\) by induction, the step uses:', options: ['\\(k(k+1)/2+1\\)', '\\(\\frac{k(k+2)}{2}+(k+1)^2\\) (squared term)', '\\((k+1)^2/2\\)', '\\(k(k+1)/2+(k+1)=(k+1)(k+2)/2\\)'], answer: 3 },
    ...
    { q: 'The conclusion of a valid induction must state:', options: ['The result is obvious', 'P(n) depends on \\(k\\), so it holds whenever P(k) holds', 'Assume P(k) true', 'By induction, P(n) holds for all required \\(n\\)'], answer: 3 },
  ],
  red: [
    { q: 'Which statement is NOT proved by induction alone?', options: ['\\(2^n>n\\) for \\(n\\ge1\\)','\\(7^n-1\\) is divisible by 6 for every positive integer \\(n\\)','\\(\\sum r=n(n+1)/2\\)','The set \\(\\mathbb{R}\\) is uncountable'], answer: 3 },
    ...
    { q: 'Proof by exhaustion is valid when:', options: ['The domain is infinite','We use contradiction for every single case','The number of cases is finite','We find one example'], answer: 2 },
...
'1.2': {
  ...
  red: [
    ...
    { q: 'Transformation \\(w=iz\\) represents:', options: ['Reflection in the imaginary axis of the Argand diagram','Dilation','Identity','Rotation \\(\\pi/2\\) anticlockwise about origin'], answer: 3 },
  ],
},
...
'1.3': {
  ...
  amber: [
    ...
    { q: 'Solving \\(Ax=b\\) with \\(\\det A\\ne0\\) gives:', options: ['\\(x=A+b\\)', '\\(x=Ab\\)', 'No solution (singular)', '\\(x=A^{-1}b\\)'], answer: 3 },
...
'1.4': {
  name: 'Vectors',
  green: [
    { q: '\\(\\mathbf{a}\\cdot\\mathbf{b}\\) equals:', options: ['A vector', '\\(|\\mathbf{a}|+|\\mathbf{b}|\\)', '\\(\\mathbf{a}\\times\\mathbf{b}\\) (a vector)', '\\(|\\mathbf{a}||\\mathbf{b}|\\cos\\theta\\)'], answer: 3 },
    { q: '\\(\\mathbf{a}\\times\\mathbf{b}\\) equals:', options: ['A scalar', '\\(\\mathbf{a}\\cdot\\mathbf{b}\\) (the same scalar)', '\\(0\\) always', 'A vector perpendicular to both'], answer: 3 },
    { q: 'If \\(\\mathbf{a}\\cdot\\mathbf{b}=0\\) and both nonzero:', options: ['Parallel to each other', 'Equal', 'Opposite', 'Perpendicular'], answer: 3 },
  ...
  amber: [
    ...
    { q: 'Scalar triple product \\(\\mathbf{a}\\cdot(\\mathbf{b}\\times\\mathbf{c})\\) represents:', options: ['Area of the parallelogram formed', 'Length', 'Angle', 'Volume of parallelepiped'], answer: 3 },
    { q: 'Three vectors are coplanar iff their scalar triple product is:', options: ['Unit', 'Positive', 'Negative', 'Zero'], answer: 3 },
    { q: 'Angle between line \\(\\mathbf{d}\\) and plane normal \\(\\mathbf{n}\\) is \\(\\alpha\\); angle between line and plane is:', options: ['\\(\\alpha\\)', '\\(\\pi-\\alpha\\) (obtuse)', '\\(2\\alpha\\)', '\\(\\pi/2-\\alpha\\)'], answer: 3 },
  ...
  red: [
    { q: 'Shortest distance from point \\(\\mathbf{p}\\) to line \\(\\mathbf{r}=\\mathbf{a}+\\lambda\\mathbf{d}\\):', options: ['\\(|\\mathbf{p}-\\mathbf{a}|\\)','\\(|(\\mathbf{p}-\\mathbf{a})\\cdot\\mathbf{d}|/|\\mathbf{d}|\\) (the projection)','\\(|\\mathbf{d}|\\)','\\(|(\\mathbf{p}-\\mathbf{a})\\times\\mathbf{d}|/|\\mathbf{d}|\\)'], answer: 3 },
    { q: 'Two skew lines: shortest distance uses:', options: ['\\(\\dfrac{|(\\mathbf{a}_2-\\mathbf{a}_1)\\cdot(\\mathbf{d}_1\\times\\mathbf{d}_2)|}{|\\mathbf{d}_1\\times\\mathbf{d}_2|}\\)','\\(|\\mathbf{a}_2-\\mathbf{a}_1|\\)','Distance from midpoint','\\(|\\mathbf{d}_1\\times\\mathbf{d}_2|\\)'], answer: 0 },
    { q: 'Area of triangle with vertices \\(\\mathbf{A},\\mathbf{B},\\mathbf{C}\\):', options: ['\\(\\mathbf{A}+\\mathbf{B}+\\mathbf{C}\\)','\\(\\tfrac{1}{2}|\\overrightarrow{AB}\\times\\overrightarrow{AC}|\\)','\\(\\tfrac{1}{2}|\\overrightarrow{AB}|\\)','\\(|\\overrightarrow{AB}\\times\\overrightarrow{AC}|\\) (the parallelogram)'], answer: 1 },
...
'1.5': {
  ...
  green: [
    ...
    { q: 'Method of differences sums telescope if \\(f(r)=\\)', options: ['\\(r\\)', '\\(r^2\\)', '\\(\\ln r\\) or \\(r\\ln r\\) terms', '\\(g(r+1)-g(r)\\)'], answer: 3 },
...
'1.6': {
  ...
  red: [
    ...
    { q: 'Substitution \\(y=x+k\\) transforms roots by:', options: ['Scaling every root by a factor of \\(k\\)','Negating','Inverting','Shifting by \\(k\\)'], answer: 3 },
...
'1.7': {
  ...
  green: [
    ...
    { q: 'A cardioid is given by \\(r=\\)', options: ['\\(a\\sin 2\\theta\\) (a rose)', '\\(a\\theta\\)', '\\(a+\\theta\\)', '\\(a(1+\\cos\\theta)\\)'], answer: 3 },
  ],
  amber: [
...
'1.8': {
  ...
  amber: [
    ...
    { q: 'Integration \\(\\int\\dfrac{dx}{\\sqrt{x^2+4}}=\\)', options: ['\\(\\operatorname{arcosh}(x/2)+C\\) for \\(x\\ge2\\)', '\\(\\arcsin(x/2)\\)', '\\(\\arctan(x/2)\\)', '\\(\\operatorname{arsinh}(x/2)+C\\)'], answer: 3 },
  ],
  red: [
...
'1.9': {
  ...
  green: [
    ...
    { q: 'Reduction formulae are typically derived using:', options: ['Substitution', 'Partial fractions of the integrand', 'Power series', 'Integration by parts'], answer: 3 },
    { q: 'Arc length of \\(y=f(x)\\) from \\(a\\) to \\(b\\):', options: ['\\(\\int(1+f^{\\prime})\\,dx\\) (no square root)', '\\(\\int f\\,dx\\)', '\\(|f(b)-f(a)|\\)', '\\(\\int\\sqrt{1+(f\')^2}\\,dx\\)'], answer: 3 },
    { q: 'Surface of revolution about \\(x\\)-axis: \\(S=\\)', options: ['\\(\\pi\\int y\\,dx\\)', '\\(2\\pi\\int y\\,dx\\)', '\\(\\pi\\int y\\sqrt{1+(y^{\\prime})^2}\\,dx\\) (no factor 2)', '\\(2\\pi\\int y\\sqrt{1+(y\')^2}\\,dx\\)'], answer: 3 },
  ],
  amber: [
...
'1.10': {
  ...
  green: [
    { q: 'An integrating factor for \\(dy/dx+P(x)y=Q(x)\\):', options: ['\\(Q(x)\\)', '\\(e^{-\\int P\\,dx}\\) (negative)', '\\(P(x)\\)', '\\(e^{\\int P(x)\\,dx}\\)'], answer: 3 },
  ...
  red: [
    { q: 'If PI trial is in CF, multiply trial by:', options: ['\\(x\\) (or \\(x^2\\) if repeated clash)','\\(x^2\\) always, whatever the clash is','Nothing','\\(e^x\\)'], answer: 0 },
    ...
    { q: 'Underdamped oscillator has aux roots:', options: ['Real positive','Real, distinct and both negative','Real repeated','Complex with negative real part'], answer: 3 },
...
'M1.1': {
  ...
  green: [
    { q: 'Dimensions of force:', options: ['\\(ML^{-1}\\) (per length)', '\\(L/T\\)', '\\(MT^{-2}\\)', '\\(MLT^{-2}\\)'], answer: 3 },
  ...
  amber: [
    ...
    { q: 'In \\(F=k m v^2/r\\), \\(k\\) must be:', options: ['\\(M\\)', '\\(T\\)', '\\(L\\) (a length)', 'Dimensionless'], answer: 3 },
    { q: 'If \\(E\\propto m^a v^b\\), dimensional analysis gives \\(a,b=\\)', options: ['\\(1,1\\)', '\\(2,1\\)', '\\(0,2\\)', '\\(1,2\\)'], answer: 3 },
  ],
  red: [
    { q: 'Dimensional analysis can find:', options: ['Exact constants','Numerical prefactors','Form of dependence on variables','Dimensionless scaling factors and their exact values'], answer: 2 },
    ...
    { q: 'Trig/log/exp arguments must be:', options: ['Dimensionless','Integers','Small','In SI base units only'], answer: 0 },
  ],
},
...
'M1.2': {
  ...
  green: [
    { q: 'Work done by constant force over displacement:', options: ['\\(F/s\\)', '\\(Fs^2\\) (squared)', '\\(F+s\\)', '\\(Fs\\cos\\theta\\)'], answer: 3 },
  ...
  amber: [
    ...
    { q: 'Elastic PE in spring of stiffness \\(k\\) at extension \\(x\\):', options: ['\\(kx\\)', '\\(k/x\\)', '\\(kx^2\\) (no factor ½)', '\\(\\tfrac{1}{2}kx^2\\)'], answer: 3 },
...
'M1.3': {
  ...
  amber: [
    ...
    { q: 'Ball of mass 2 kg moving 5 m/s hits stationary 3 kg, \\(e=0\\). Combined speed:', options: ['\\(5\\)', '\\(3\\)', '\\(1.67\\) m/s', '\\(2\\) m/s'], answer: 3, explanation: '10=5v → v=2' },
  ...
  red: [
    ...
    { q: 'For successive bounces with \\(e<1\\), total distance converges because:', options: ['Gravity decreases with each successive bounce','\\(e^2>1\\)','Geometric series with ratio \\(e^2<1\\)','No friction'], answer: 2 },
...
'M1.4': {
  ...
  green: [
    ...
    { q: 'Centripetal force is directed:', options: ['Tangentially (forward)', 'Outward', 'At 45°', 'Towards the centre'], answer: 3 },
    { q: 'Period of circular motion:', options: ['\\(\\omega\\)', '\\(1/\\omega\\) (seconds)', '\\(r\\)', '\\(2\\pi/\\omega\\)'], answer: 3 },
    { q: 'Units of angular velocity:', options: ['m/s (m s⁻¹)', 's', 'Hz', 'rad/s'], answer: 3 },
  ],
  amber: [
    { q: 'Conical pendulum: \\(\\tan\\theta=\\)', options: ['\\(v/g\\)', '\\(r/g\\)', '\\(\\omega g/r\\) (inverted)', '\\(r\\omega^2/g\\)'], answer: 3 },
    { q: 'Banked track design speed: \\(\\tan\\alpha=\\)', options: ['\\(v\\)', '\\(g/v^2\\) (inverted)', '\\(r/v\\)', '\\(v^2/(rg)\\)'], answer: 3 },
  ...
  red: [
    { q: 'At top of vertical loop, tension is 0 when:', options: ['\\(v=0\\) (at rest)','Always','Never','\\(v^2=gr\\)'], answer: 3 },
    { q: 'Conservation of energy in vertical circle gives:', options: ['\\(\\tfrac{1}{2}v^2+gh=\\) const','No constraint','Energy is lost to friction at every point on the circle','KE stays constant'], answer: 0 },
...
'M1.5': {
  ...
  green: [
    { q: 'For a system: \\(\\bar{x}=\\)', options: ['Mean of \\(x_i\\)', '\\(\\sum x_i\\)', '\\(n\\sum m_i x_i/\\sum x_i\\)', '\\(\\sum m_ix_i/\\sum m_i\\)'], answer: 3 },
  ...
  amber: [
    ...
    { q: 'Object suspended freely: CoM sits:', options: ['Above', 'On suspension point', 'Off to one side of the suspension point', 'Directly below the suspension point'], answer: 3 },
  ...
  red: [
    ...
    { q: 'Body slides before toppling if \\(\\mu < a/h\\) where \\(a\\) is half-base, \\(h\\) is CoM height — slide vs. topple angle:', options: ['Depends only on the mass of the body','Always topples','Always slides','Compare \\(\\mu\\) with \\(a/h\\)'], answer: 3 },
    { q: 'For a region with \\(y=f(x)\\): \\(\\bar{x}=\\)', options: ['Average','0','\\(\\int xf\\,dx/\\int f\\,dx\\)','Same as \\(\\bar{y}\\) by symmetry'], answer: 2 },
    { q: 'Solid of revolution \\(y=f(x)\\) about \\(x\\)-axis: \\(\\bar{x}=\\)', options: ['Same as for the lamina with the same boundary','\\(\\int f\\,dx\\)','\\(\\int x\\cdot y^2\\,dx / \\int y^2\\,dx\\)','Centroid of curve'], answer: 2 },
  ],
},
...
'S1.1': {
  ...
  green: [
    ...
    { q: '\\(E(X)=\\)', options: ['\\(\\sum P(X=x)\\) (equals 1)', '\\(\\max x\\)', '\\(0\\)', '\\(\\sum x P(X=x)\\)'], answer: 3 },
    { q: '\\(\\operatorname{Var}(X)=\\)', options: ['\\(E(X)^2\\)', '\\(|\\mu|\\)', '\\(\\sigma\\)', '\\(E(X^2)-\\mu^2\\)'], answer: 3 },
    { q: '\\(E(aX+b)=\\)', options: ['\\(aE(X)\\)', '\\(E(X)+b\\) (no \\(a\\))', '\\(a+b\\)', '\\(aE(X)+b\\)'], answer: 3 },
    { q: '\\(\\operatorname{Var}(aX+b)=\\)', options: ['\\(a\\operatorname{Var}(X)+b\\) (linear)', '\\(\\operatorname{Var}(X)+b\\)', '\\(a+b\\)', '\\(a^2\\operatorname{Var}(X)\\)'], answer: 3 },
  ],
  amber: [
    { q: 'For \\(X\\sim B(n,p)\\): \\(E(X)=\\)', options: ['\\(p\\)', '\\(nq\\)', '\\(n(1-p)\\)', '\\(np\\)'], answer: 3 },
    { q: 'For \\(X\\sim B(n,p)\\): \\(\\operatorname{Var}(X)=\\)', options: ['\\(n^2p(1-p)\\)', '\\(np\\)', '\\(np^2\\)', '\\(np(1-p)\\)'], answer: 3 },
    ...
    { q: 'Uniform on \\(\\{1,\\ldots,n\\}\\): \\(\\operatorname{Var}(X)=\\)', options: ['\\(n/2\\)', '\\((n+1)/12\\) (for \\(n\\) values)', '\\(n^2/12\\)', '\\((n^2-1)/12\\)'], answer: 3 },
  ],
  red: [
    ...
    { q: '\\(\\operatorname{Var}(X+Y)=\\operatorname{Var}(X)+\\operatorname{Var}(Y)+\\)', options: ['\\(\\mu_X\\mu_Y\\)','\\(\\operatorname{Cov}(X,Y)\\) (only once)','0','\\(2\\operatorname{Cov}(X,Y)\\)'], answer: 3 },
...
'S1.2': {
  ...
  green: [
    { q: 'Poisson pmf:', options: ['\\(\\binom{n}{r}p^r(1-p)^{n-r}\\) (the binomial pmf)', '\\(e^r\\lambda/r!\\)', '\\(r!e^{-\\lambda}\\)', '\\(\\dfrac{e^{-\\lambda}\\lambda^r}{r!}\\)'], answer: 3 },
...
'S1.3': {
  ...
  green: [
    ...
    { q: '\\(r=1\\) means:', options: ['No correlation', 'Negative linear correlation', 'Quadratic', 'Perfect positive linear'], answer: 3 },
    ...
    { q: 'Regression line \\(y=a+bx\\) through \\((\\bar{x},\\bar{y})\\) passes through:', options: ['Origin', 'The highest point on the scatter graph', 'Median', '\\((\\bar{x},\\bar{y})\\)'], answer: 3 },
  ],
  amber: [
    { q: 'PMCC formula uses:', options: ['The sums of \\(x\\) and \\(y\\) only, not products', 'Ranks', 'Percentiles', '\\(S_{xy}/\\sqrt{S_{xx}S_{yy}}\\)'], answer: 3 },
    { q: 'Spearman formula (no ties): \\(\\rho_s=1-\\)', options: ['\\(\\sum d/n\\) (the mean difference of ranks)', '\\(d^2\\)', 'Nothing', '\\(6\\sum d^2/(n^3-n)\\)'], answer: 3 },
  ...
  red: [
    ...
    { q: 'PMCC is unchanged under:', options: ['Absolute value','Linear transformations of \\(x\\) and \\(y\\) (sign preserved)','Negating \\(y\\) while leaving \\(x\\) unchanged, so the sign flips','Adding outliers'], answer: 1 },
    { q: 'Spearman detects:', options: ['Only linear','Only linear relationships between the variables','Causation','Monotonic relationships'], answer: 3 },
  ],
},
...
'S1.4': {
  ...
  green: [
    ...
    { q: 'Contingency table \\(r\\times c\\): df = ', options: ['\\(rc\\)', '\\(r+c\\)', '\\(rc-1\\) (all cells)', '\\((r-1)(c-1)\\)'], answer: 3 },
    { q: 'Yates\' correction is for:', options: ['Any df', 'Large df', 'Very small samples with any number of df', '\\(df=1\\) with small counts'], answer: 3 },
  ],
  amber: [
    { q: 'Test is valid when:', options: ['\\(E_i=0\\) OK', '\\(O_i\\ge5\\)', 'Any \\(O\\)', 'All \\(E_i\\ge5\\) (pool if needed)'], answer: 3 },
    { q: 'Expected under independence: \\(E_{ij}=\\)', options: ['\\(\\text{row}_i\\)', '\\(\\text{col}_j\\)', '\\(N/(rc)\\) for every cell', '(row × col)/\\(N\\)'], answer: 3 },
    { q: 'Critical value exceeded → ', options: ['Accept \\(H_0\\)', 'Nothing', 'Always accept \\(H_1\\)', 'Reject \\(H_0\\)'], answer: 3 },
    { q: 'Parameters estimated from data reduce df by:', options: ['0', '1 only', '2 each, always, per parameter', '1 per parameter'], answer: 3 },
  ...
  red: [
    { q: 'For \\(2\\times 2\\) table with Yates:', options: ['\\(\\sum(O-E)^2/E\\) (no correction)','Subtract 1','\\(\\sum(|O-E|-0.5)^2/E\\)','Same as large'], answer: 2 },
    ...
    { q: 'A very small \\(\\chi^2\\) (much smaller than expected) might indicate:', options: ['Too many d.f.','Data too good a fit — possibly fudged','Obviously correct data, with a perfect model','Test failure'], answer: 1 },
  ],
},
...
'S1.5': {
  ...
  green: [
    { q: 'Non-parametric tests:', options: ['Assume normality', 'Need large \\(n\\)', 'Test the variance only, assuming the data are normal', 'Make no or few distributional assumptions'], answer: 3 },
    { q: 'Sign test uses:', options: ['Sum of ranks', 'Mean', 'Variance', 'Number of positive vs. negative signs'], answer: 3 },
    { q: 'Wilcoxon signed-rank test ranks:', options: ['Raw values of the data', 'Squares', 'Proportions', 'Absolute differences'], answer: 3 },
    ...
    { q: 'Null hypothesis for paired sign test:', options: ['Mean of the differences = 0', 'Variance = 1', 'Data normal', 'Median of differences = 0'], answer: 3 },
  ],
  amber: [
  ...
  red: [
    ...
    { q: 'Ties are handled in Wilcoxon by:', options: ['Random assignment of ranks to tied values','Rank 0','Ignoring','Assigning average of tied ranks'], answer: 3 },
  ],
},
...
'A1.1': {
  ...
  green: [
    { q: 'A graph has:', options: ['Only edges', 'Only vertices (nodes)', 'Numbers', 'Vertices and edges'], answer: 3 },
    { q: 'Degree of a vertex = ', options: ['Coordinate', 'Weight', 'Number of vertices', 'Number of incident edges'], answer: 3 },
    { q: 'Complete graph \\(K_n\\) has edges:', options: ['\\(n\\)', '\\(n^2\\) (each pair twice)', '\\(2n\\)', '\\(n(n-1)/2\\)'], answer: 3 },
  ...
  amber: [
    ...
    { q: 'Semi-Eulerian (Eulerian trail) needs exactly:', options: ['0 odd-degree vertices at all', 'All odd', '1 odd', '2 odd-degree vertices'], answer: 3 },
  ...
  red: [
    ...
    { q: 'Planar graph satisfies \\(E\\le\\)', options: ['\\(V\\)','\\(V^2\\)','\\(V-1\\) (a tree)','\\(3V-6\\)'], answer: 3 },
    { q: 'Smallest non-planar graphs:', options: ['\\(K_3\\)','\\(K_5\\) and \\(K_{3,3}\\)','Tree','\\(K_4\\) and \\(K_{2,3}\\) only'], answer: 1 },
  ],
},
...
'A1.2': {
  ...
  green: [
    ...
    { q: 'Prim\'s algorithm starts:', options: ['From any edge', 'At a random vertex each step, adding the lightest arc anywhere', 'Sorts edges', 'At a single vertex, extends by nearest'], answer: 3 },
    { q: 'Kruskal\'s algorithm processes edges:', options: ['In any order', 'Random', 'By vertex degree, from highest to lowest, skipping loops', 'In increasing order of weight, skipping cycles'], answer: 3 },
    { q: 'Dijkstra finds:', options: ['MST', 'An Eulerian trail through every arc', 'Hamilton cycle', 'Shortest path from source'], answer: 3 },
    { q: 'Dijkstra assumes:', options: ['Planar graph', 'Negative weights are allowed', 'Unweighted only', 'Non-negative weights'], answer: 3 },
  ],
  amber: [
    { q: 'Route inspection: if graph has all even degrees, optimal tour weight = ', options: ['Half total', 'Plus the repeated shortest paths', '0', 'Total edge weight (Eulerian)'], answer: 3 },
    ...
    { q: 'Nearest-neighbour TSP heuristic: start and:', options: ['Go to farthest', 'Random choice', 'Always backtrack to the start', 'Go to nearest unvisited'], answer: 3 },
    { q: 'Dijkstra labels: each vertex gets a:', options: ['Color', 'Two sublabels, fixed from the start and never updated', 'Predecessor only', 'Temporary label updated until made permanent'], answer: 3 },
  ],
  red: [
    ...
    { q: 'Dijkstra\'s complexity is typically:', options: ['\\(O(2^V)\\) (exponential in \\(V\\))','\\(O(1)\\)','\\(O(V)\\)','\\(O(E\\log V)\\) or \\(O(V^2)\\)'], answer: 3 },
    { q: 'Route inspection needs:', options: ['Connected graph','MST','Planarity of the network','Hamiltonicity'], answer: 0 },
  ],
},
...
'A1.3': {
  ...
  green: [
    ...
    { q: 'Optimal solution of an LP lies at:', options: ['The centre of the feasible region', 'Any point', 'Edge only', 'A vertex (basic solution)'], answer: 3 },
    { q: 'The Simplex method:', options: ['Random walk', 'Uses graphs', 'Solves nonlinear problems by following gradients', 'Moves vertex-to-vertex improving objective'], answer: 3 },
  ],
  amber: [
    { q: 'Slack variable converts \\(\\le\\) into:', options: ['\\(=0\\)', '\\(\\geq\\)', 'Nothing', 'Equality with non-negative slack added'], answer: 3 },
    { q: 'In Simplex, pivot column chosen as:', options: ['Any column', 'Smallest entry', 'The largest ratio in the constraint rows', 'Most negative entry in objective row'], answer: 3 },
    { q: 'Pivot row chosen by:', options: ['Largest', 'Random', 'The column containing a zero in the objective row', 'Smallest non-negative ratio (RHS / pivot col)'], answer: 3 },
    { q: 'If objective row has no negative entries:', options: ['Iterate again, choosing a new pivot', 'Infeasible', 'Error', 'Optimal solution reached'], answer: 3 },
  ...
  red: [
    ...
    { q: 'Artificial variables are used for:', options: ['\\(\\geq\\) or \\(=\\) constraints (Big-M method)','Slack','\\(\\le\\) constraints (as with slack variables)','Linear only'], answer: 0 },
  ],
},
...
'A1.4': {
  ...
  green: [
    { q: 'Activity-on-arc network shows:', options: ['Resources needed by each event in turn', 'Only costs', 'Random paths', 'Activities as arcs, events at nodes'], answer: 3 },
    { q: 'Earliest event time computed via:', options: ['Backward pass from the end', 'Random', 'Subtraction', 'Forward pass from start'], answer: 3 },
    { q: 'Latest event time computed via:', options: ['Forward pass from the start', 'Random', 'Addition', 'Backward pass from end'], answer: 3 },
  ...
  amber: [
    { q: 'Total float of activity (i,j):', options: ['\\(t_j - t_i\\)', 'Duration', 'Earliest − latest event time', '\\(LT_j - ET_i -\\) duration'], answer: 3 },
    ...
    { q: 'Precedence table describes:', options: ['The durations of the activities only, in order', 'Resources', 'Events', 'Which activities must precede others'], answer: 3 },
  ],
  red: [
    { q: 'PERT uses:', options: ['Uniform','Normal','Deterministic only','Three-point (optimistic, most likely, pessimistic) estimates'], answer: 3 },
    { q: 'Crashing an activity:', options: ['Reduces the float only, without changing duration','Shortens duration at extra cost','Delays project','Has no cost'], answer: 1 },
    { q: 'Independent float is:', options: ['Always positive','Same as total','Greater than the total float for that activity','Float usable without affecting successors'], answer: 3 },
    { q: 'A project\'s duration equals:', options: ['The critical-path length','Shortest path','The sum of all the activity durations','Mean duration'], answer: 0 },
  ],
},
...
'A1.5': {
  name: 'Simulation',
  green: [
    { q: 'Simulation is useful when:', options: ['Analytical is easy', 'Random variation is not involved in the problem', 'Only small problems', 'Problem is too complex for analytical solution'], answer: 3 },
    ...
    { q: 'Monte Carlo simulation uses:', options: ['One trial', 'Deterministic inputs only', 'Graph theory', 'Repeated random trials'], answer: 3 },
    ...
    { q: 'Simulation outputs typically:', options: ['Always exact', 'Single exact values with no spread', 'No variance', 'Estimates with sampling variance'], answer: 3 },
  ],
  amber: [
    ...
    { q: 'Seeding ensures:', options: ['Only fast execution of the model', 'Randomness', 'No randomness', 'Reproducibility of simulation'], answer: 3 },
  ...
  red: [
    { q: 'Convergence of Monte Carlo estimate:', options: ['\\(O(1/n)\\) (linear)','\\(O(1/\\sqrt{n})\\)','\\(O(\\log n)\\)','\\(O(1)\\)'], answer: 1 },
    ...
    { q: 'A deterministic queue system has:', options: ['Exponential arrivals and random service times','Same results','Random outputs','Fixed arrival and service times'], answer: 3 },
