...
(Subject_Board|gcse / |alevel — the suffixed key wins). DIAG_TOPICS lives in
topics-config.js only — add the topic list there ONCE; Dashboard.html,
breakdown.html and medals.html all read it from there. If the spec has
...
| Topics | `topics-config.js` — `DIAG_TOPICS` (emoji, colour, `examinedTotal`); read by `Dashboard.html`, `breakdown.html`, `medals.html` |
