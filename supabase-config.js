...
// ─────────────────────────────────────────────────────────────────────────────
// LEVEL SUFFIX — the single definition of how a level becomes a map key suffix.
//
...
// suffix applies used to be copy-pasted into a dozen files, which drifted:
...
//
// levelSuffix(level?)          -> 'alevel' | 'gcse'   (reads cached_level if
//                                 no level is passed)
// levelLookup(map, base, level?) -> map[base|suffix] ?? map[base]
//
// Lives in supabase-config.js because that is the one script every page loads,
// and it loads before flashcards-config.js, practice-bank-config.js,
// medals-engine.js and predicted-grade.js on all of them.
// ─────────────────────────────────────────────────────────────────────────────
(function () {
...
(function () {
  ...
  function currentLevel() { return window.levelSuffix(); }
